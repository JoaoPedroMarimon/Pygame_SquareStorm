...
def criar_inimigos(numero_fase):
    ...
    """
    inimigos = []
    for i in range(numero_fase):
        # Posicionar inimigos em locais diferentes
        pos_x = LARGURA - 130 - (i * 20)
        pos_y = ALTURA // 2 + ((-1)**i * i * 30)  # Alternando posições acima/abaixo
        
        # Ajustar para manter dentro da tela
        pos_y = max(50, min(pos_y, ALTURA - 90))
        
        # Cores ligeiramente diferentes para cada inimigo
        cor_base = list(VERMELHO)
        cor_base[1] = min(255, cor_base[1] + i * 15)  # Variação no verde
        cor_base[2] = min(255, cor_base[2] + i * 10)  # Variação no azul
        cor_inimigo = tuple(cor_base)
        
        # Velocidade crescente com a fase, mas decrescente com a quantidade de inimigos
        velocidade_base = VELOCIDADE_INIMIGO_BASE + (numero_fase * 0.2)
        velocidade_inimigo = max(3.0, velocidade_base - (i * 0.3))
...
def jogar_fase(tela, relogio, numero_fase, gradiente_jogo, fonte_titulo, fonte_normal):
    ...
    # Tempo para mostrar a transição de fase
    tempo_transicao = DURACAO_TRANSICAO_FASE  # Frames
    ...
    while rodando:
        ...
        for evento in pygame.event.get():
            ...
            if not mostrando_inicio:
                if evento.type == pygame.KEYDOWN:
                    ...
                    # Atirar com espaço (para a direita)
                    if evento.key == pygame.K_SPACE:
                        jogador.atirar(tiros_jogador)
                    
                    # Atirar com as setas (nas diagonais)
        ...
        for idx, inimigo in enumerate(inimigos):
            tempo_movimento_inimigos[idx] = atualizar_IA_inimigo(
                inimigo, idx, jogador, tiros_jogador, inimigos, 
                tempo_atual, tempo_movimento_inimigos, intervalo_movimento, 
                numero_fase, tiros_inimigo, movimento_x, movimento_y
            )
        ...
        if todos_derrotados:
            # Mostrar transição para próxima fase
            if tempo_transicao > 0:
                # Continuar renderizando o jogo com texto de vitória da fase
                ...
                # Desenhar transição
                desenhar_transicao_fase(tela, numero_fase, tempo_transicao, fonte_titulo, fonte_normal)
